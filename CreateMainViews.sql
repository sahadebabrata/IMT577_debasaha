...
Assignment: Module 8
Date: 05/22/2022
Description: Script for creating secure views for addressing business questions. 
*****************************************/

--VIEW FOR STORE-CHANNEL TARGETS 
CREATE OR REPLACE SECURE VIEW VW_STORE_SALES_TARGET AS
SELECT
	ds."SourceStoreID" , ds."StoreNumber" , dc."SourceChannelID" , dc."ChannelName" , dd."YEAR" , SUM(fsst."SalesTargetAmount") AS "StoreSalesTargetAmount"
	--ChannelID is necessary since we are measuring Store sales, which means we need to exclude Reseller targets 
FROM FACT_SRCSALESTARGET fsst, DIM_STORE ds, DIM_CHANNEL dc,DIM_DATE dd
WHERE
	fsst."DimStoreID" = ds."DimStoreID"
	AND dc."DimChannelID" = fsst."DimChannelID" 
	AND dd.DATE_PKEY = fsst."DimTargetDateID" 
GROUP BY ds."SourceStoreID" ,ds."StoreNumber" , dc."SourceChannelID" , dc."ChannelName" , dd."YEAR";
...
-- VIEW FOR VIEW_STORE_LOCATION_DETAILS
CREATE OR REPLACE SECURE VIEW VIEW_STORE_LOCATION_DETAILS AS 
SELECT ds."SourceStoreID" , ds."StoreNumber" , dl."Region" , dl."Country" 
FROM DIM_STORE ds, DIM_LOCATION dl
WHERE dl."DimLocationID" = ds."DimLocationID" ;

-- VIEW FOR AGGREGATE SALES DATA  
CREATE OR REPLACE SECURE VIEW VW_STORE_AGG_SALES_DATA AS
SELECT  dp."SourceProductID" , dp."ProductName", dp."SourceProductCategoryID" , dp."ProductCategory" , ds."SourceStoreID" , ds."StoreNumber" , dc."SourceChannelID" , dc."ChannelName" , dd.YEAR, 
...
GROUP BY  dp."SourceProductID" , dp."ProductName", dp."SourceProductCategoryID" , dp."ProductCategory" , ds."SourceStoreID", ds."StoreNumber" , dc."SourceChannelID" , dc."ChannelName" , dd.YEAR;


-- VIEW FOR PRODUCT AGGREGATE SALES BY DAY OF WEEK   
CREATE OR REPLACE SECURE VIEW VW_STORE_AGG_SALES_BY_DAY AS
SELECT  dp."SourceProductID" , dp."ProductName", dp."SourceProductCategoryID" , dp."ProductCategory" , ds."SourceStoreID",  ds."StoreNumber" , dc."SourceChannelID" , dc."ChannelName" , dd.DAY_NUM_IN_WEEK, dd.DAY_NAME , dd.DAY_ABBREV , 
SUM(fsa."SaleQuantity") AS "TotalSaleQuantity", SUM(fsa."SaleAmount") AS "TotalSaleAmount", SUM(fsa."SaleExtendedCost") AS "TotalSaleExtendedCost", 
SUM(fsa."SaleTotalProfit") AS "TotalSaleProfit"
FROM FACT_SALESACTUAL fsa, DIM_STORE ds, DIM_DATE dd, DIM_PRODUCT dp, DIM_CHANNEL dc
WHERE fsa."DimStoreID" = ds."DimStoreID" 
